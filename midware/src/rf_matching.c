...
volatile float Best_vswr =99.0f;
volatile uint8_t best_comb = 0;
volatile uint16_t ADC_0 = 0;
volatile uint16_t ADC_1 = 0;
volatile float fwd_raw = 0.0f;
volatile float ref_raw = 0.0f;
volatile float vswr = 0.0f;
float vswr_viewer = 0.0f;
...
static float Voltage_to_dbm(uint16_t adc_raw) {
    ...
    // 你的逻辑: v_ad8310 = (v_pin - 1.67) * 1.2
    // 反推公式: v_ad8310 = (v_pin / 1.2) + 1.67
    float v_ad8310 = (v_pin / 1.2f) + 1.67f;
    ...
    // 4. 定向耦合器衰减的补偿，50 dB 的衰减
     const float COUPLER_ATTEN_DB = 50.0f; 

    // 根据公式: Pin = (V_ad8310 / Slope) + Intercept [cite: 786]，外加定向耦合器的补偿
    float dbm = (v_ad8310 / V_SLOPE) + P_INTERCEPT + COUPLER_ATTEN_DB;
...
//    Best_vswr = 0.0f;//在遍历前要将前一次留下的Best_vswr清零
   ...
   Best_vswr = 99.0f;//在遍历前要将前一次留下的Best_vswr清零
